...
/* only usable for MPEG-PS streams */
void av_update_estimate_timings_from_pts(AVFormatContext *ic)
{
	AVPacket pkt1, *pkt = &pkt1;
    AVStream *st;
    int read_size, i, ret;
	...
	//save and restore
	AVPacketList *Pre_packet_buffer_list = NULL,*Pre_raw_packet_buffer_list = NULL;
	AVPacketList *Pre_packet_buffer_end	 		= ic->packet_buffer_end;
	AVPacketList *Pre_raw_packet_buffer_end	 	= ic->raw_packet_buffer_end;
	int			  Pre_raw_packet_buffer_remaining_size = ic->raw_packet_buffer_remaining_size;
	AVPacketList      *pPacket = NULL;
    ...
    //flush_packet_queue(ic);
	{
	    ...
	    for(;;) 
		{
	        pktlist = ic->packet_buffer;
	        if (!pktlist)
	            break;
	        ic->packet_buffer = pktlist->next;
	       // av_free_packet(&pktl->pkt);
	       // av_free(pktl);
	       if(Pre_packet_buffer_list == NULL)
	       {
	       		Pre_packet_buffer_list = pktlist;
	       }
		   else
		   {
				pPacket->next 	= pktlist;		
		   }
			pPacket			= pktlist;	
	    }
	    while(ic->raw_packet_buffer)
		{
	        pktlist = ic->raw_packet_buffer;
	        ic->raw_packet_buffer = pktlist->next;

	        if(Pre_raw_packet_buffer_list == NULL)
	        {
	       		Pre_raw_packet_buffer_list = pktlist;
	        }
		    else
		    {
				pPacket->next 	= pktlist;
...

	{
	    ...
	    for(;;) 
	    {
	    	pktlist = Pre_packet_buffer_list;
			if(!pktlist)
				break;

			Pre_packet_buffer_list = pktlist->next;

			if(ic->packet_buffer == NULL)
			{
				ic->packet_buffer = pktlist;
			}
			else
			{
				pPacket->next	  = pktlist;
		...
		while(Pre_raw_packet_buffer_list)
		{
			pktlist = Pre_raw_packet_buffer_list;
			Pre_raw_packet_buffer_list = pktlist;
			if(ic->raw_packet_buffer == NULL)
			{	
				ic->raw_packet_buffer = pktlist;
			}
			else
			{
				pPacket->next			= pktlist;
		...
		ic->packet_buffer_end=Pre_packet_buffer_end;
	    ic->raw_packet_buffer_end= Pre_raw_packet_buffer_end;
	    ic->raw_packet_buffer_remaining_size = Pre_raw_packet_buffer_remaining_size;
    }
}
