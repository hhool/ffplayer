...
void     CAndroidAudioTrackRender::Start() {
  ...
  if (mTrack && mState != AR_RUNNING) {
    ...
    DEBUG("mBufferLength %d,mBufferLimit %d,mBufferUsed %d,mBufferFillFirst 0x%x, mBufferFillLast 0x%x,mBufferFreeFirst 0x%x,mBufferFreeLast 0x%x mCurBuffer 0x%x,mFillPos %d,mBytes %d", \
